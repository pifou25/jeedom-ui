...
for (const [schemaName, schema] of Object.entries(schemas)) {
  // si le nom de schéma ne termine pas par "param" on l'ajoute à la liste des schémas Response
...
for (const [schemaName, schema] of Object.entries(schemas)) {
  ...
  const responseEmpty = !schemasResponse[responseName] || !schemasResponse[responseName].properties || Object.keys(schemasResponse[responseName].properties).length === 0;
  if (!schemasResponse[responseName]) {
    console.log(`ℹ️ Le schéma ${responseName} n'existe pas. La réponse de la méthode ${methodName} sera de type 'any'.`);
  } else if (schemasResponse[responseName].properties && Object.keys(schemasResponse[responseName].properties).length === 0) {
    console.log(`ℹ️ Le schéma ${responseName} n'a pas de propriétés. La réponse de la méthode ${methodName} sera de type 'any'.`);
  }
  const response = responseEmpty ? 'any' : responseName;
